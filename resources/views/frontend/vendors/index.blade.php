            ...
            <div data-mobile-filter-panel class="fixed inset-0 z-[200] hidden">
                ...
                <div class="absolute inset-y-0 left-0 w-[85%] max-w-sm overflow-y-auto bg-white p-6 shadow-2xl">
                    ...
                    <form method="GET" action="{{ $category ? route('vendors.category', $category->slug) : route('vendors.index') }}">
                        ...
                        <button type="submit" class="bd-btn-primary w-full justify-center mt-6 py-3.5 text-base font-bold shadow-sm">Terapkan Filter</button>
                    </form>
                </div>
            </div>
                    ...
                    <div class="mt-4 flex flex-wrap items-center gap-2">
                        @foreach ($activeFilters as $filter)
                            <a href="{{ $filterBaseUrl . '?' . active_filter_query([], [$filter['type']]) }}"
                               class="inline-flex items-center gap-1.5 rounded-md border border-ink-200 bg-white px-3.5 py-1.5 text-sm font-medium text-ink-700 shadow-xs transition-all hover:border-rose-400 hover:bg-rose-50">
                                {{ $filter['label'] }}
                                <svg class="h-3.5 w-3.5 text-ink-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/></svg>
                            </a>
                        @endforeach
                        <a href="{{ $resetUrl }}"
                           class="inline-flex items-center gap-1.5 rounded-md bg-ink-100 px-3.5 py-1.5 text-sm font-bold text-ink-700 hover:bg-rose-50 hover:text-rose-600 transition-colors">
                            Reset Semua
                        </a>
                    </div>
                @endif
